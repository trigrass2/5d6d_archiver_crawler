...
{
	fetch_url = %1%	;http://fable.5d6d.net/archiver/tid-10511.html
	filepath = %2%	;fable
	SplitPath, fetch_url, file_name, web_dir, file_ext, save_url, domain_url
	;file_name:tid-10511.html
	...
	;-----
		...
		Loop
		{
			...
			{
				FileAppend, %web_dir%/%res_2%`r`n, %save_url%.txt ;帖子页面入口文件 tid-10511
			}
		}
		...
		IfExist, %save_url%.txt
		{
			IfExist, get_tid_page.exe
				RunWait, get_tid_page.exe %web_dir%/%save_url%-page-1.html %filepath%, %A_scriptdir%, Max UseErrorLevel
			else
